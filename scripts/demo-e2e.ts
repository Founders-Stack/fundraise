...
// unsigned "transaction" (there is nothing to actually sign/send locally) — the module comment in
// apps/web/lib/chain/fake.ts explicitly reserves `fakeSwap()` for this ("used by e2e scripts, not by
// ports") as the fake-mode equivalent of "the investor's wallet signs and sends the tx". So each
// trade below is exercised twice, exactly mirroring what a real devnet run does (scripts/chain/ports-
// smoke.ts): first the real HTTP quote/swap endpoints (fee breakdown, the NotEligible warning, the
// unsigned tx), then `fakeSwap` standing in for "the wallet signed and the validator confirmed it" —
// which is also where the fs_allowlist hook's NotEligible failure actually happens in fake mode.
...
async function main() {
  ...
  try {
    ...
    assertEqual("Alice holds 100,000 tokens", aliceRow1.tokensDisplay, "100,000");
    ...
    assertEqual("Q3 rights pool = $40,000", q3Report.body.distribution.rightsPool, "40000");
    assertEqual("Q3 per token = $0.04", q3Report.body.distribution.perToken, "0.04");
    ...
    assertEqual("Q3 unallocated (pool) = $36,000", q3Snapshot.body.unallocated.total, "36000");
    ...
    assertEqual("Q3 snapshot: Alice previewed payout = 4000", q3AliceSnap.payout, "4000");
    assertEqual("Q3 snapshot: Alice tokens = 100,000", q3AliceSnap.tokens, "100,000");
    ...
    assertEqual("Alice receives $4,000 USDC for Q3", q3AlicePaid.payout, "4000");
    ...
    assertEqual("holders table: Alice now holds 60,000", aliceRow2.tokensDisplay, "60,000");
    assertEqual("holders table: Bob now holds 40,000", bobRow2.tokensDisplay, "40,000");
    ...
    assertEqual("Q4 rights pool = $45,000", q4Report.body.distribution.rightsPool, "45000");
    assertEqual("Q4 per token = $0.045", q4Report.body.distribution.perToken, "0.045");
    ...
    assertEqual("Q4 snapshot: Alice tokens = 60,000", q4AliceSnap.tokens, "60,000");
    assertEqual("Q4 snapshot: Bob tokens = 40,000", q4BobSnap.tokens, "40,000");
    ...
    assertEqual("Alice receives $2,700 USDC for Q4 (60,000 tokens)", q4AlicePaid.payout, "2700");
    assertEqual("Bob receives $1,800 USDC for Q4 — the units Alice sold now pay Bob", q4BobPaid.payout, "1800");
    ...
    assertEqual("yield.lastPerToken = 0.045", history.body.yield.lastPerToken, "0.045");
    assertEqual("yield.ttmPerToken = 0.085", history.body.yield.ttmPerToken, "0.085");
    assertEqual("yield.annualizedRunRatePerToken = 0.18", history.body.yield.annualizedRunRatePerToken, "0.18");
  ...
  History: ${executedPeriods.map((d: { periodLabel: string; totalAllocated: string }) => `${d.periodLabel} EXECUTED $${d.totalAllocated}`).join("  |  ")}
  ${"-".repeat(64)}`);
