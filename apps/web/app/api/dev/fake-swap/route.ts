...
import { getIssuanceOr404 } from "@/lib/server/issuance";
...
export async function POST(req: Request) {
  ...
  return handle(async () => {
    ...
    const issuance = await getIssuanceOr404(issuanceId);
    if (!issuance.dbcPool) throw new HttpError(409, "issuance_pending", `Issuance ${issuanceId} has no market yet`);

    const { fakeSwap } = await import("@/lib/chain/fake");
    try {
      const { signature } = fakeSwap(issuance.dbcPool, owner, side, tokens);
