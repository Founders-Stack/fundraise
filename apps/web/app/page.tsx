...
async function loadRows() {
  const issuances = await prisma.issuance.findMany({ orderBy: { createdAt: "asc" } });
  return Promise.all(
                    ...
                    <tr key={i.id} className="group hover:bg-surface/60 [&>td]:px-4 [&>td]:py-4">
                      <td>
                        <Link href={`/market/${i.id}`} className="flex items-center gap-3">
                          <TokenMark symbol={i.symbol} />
                          <div>
                            <div className="font-medium">{i.symbol}</div>
                            <div className="text-xs text-muted-foreground">{i.issuerName}</div>
                          </div>
                        </Link>
                      </td>
                      <td className="text-muted-foreground">
                        {bps(i.poolPercentageBps)} of {freqWord(i.distributionFrequency)} distributable cash flow
                      </td>
