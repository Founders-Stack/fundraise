...
export async function launch(overrides: Record<string, unknown> = {}) {
  ...
  return c.body as { issuanceId: string; agreementHash: string; dbcPool: string; baseMint: string; nextRecordDate: string };
}
...
export function signer(issuanceId: string, agreementHash: string) {
  ...
  return { wallet, signature: sign(agreementAcceptanceMessage(agreementHash, issuanceId)), sign };
}

/** Full investor onboarding through the public participants API (verified, eligible, signed, allowlisted). */
export async function onboard(issuance: { issuanceId: string; agreementHash: string }, displayName: string) {
  const { wallet, signature } = signer(issuance.issuanceId, issuance.agreementHash);
  const r = await call(
    participantsPOST,
    post({ wallet, displayName, verified: true, eligible: true, agreementHash: issuance.agreementHash, signature }, {}),
    issuance.issuanceId,
  );
